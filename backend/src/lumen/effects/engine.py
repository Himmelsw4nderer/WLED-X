...
if TYPE_CHECKING:
    from lumen.preview_window import LivePreviewWindow
...
class RenderLoop:
    ...
    def _maybe_open_preview_window(self) -> None:
        ...
        try:
            ...
            logger.info("local preview window enabled (LUMEN_PREVIEW_WINDOW)")
        except Exception:
    ...
    def _maybe_attach_playlist_runner(self) -> None:
        ...
        if self._playlist_runner is not None:
            return
        try:
            from lumen.effects.playlist_runner import PlaylistRunner
        except ImportError:
            return
