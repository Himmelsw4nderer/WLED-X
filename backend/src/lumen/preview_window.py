...
* **`LivePreviewWindow`** -- an in-process Tk window the running backend can
  open (set `LUMEN_PREVIEW_WINDOW=true`). It's fed straight from the render
...
* **the `python -m lumen.preview_window` CLI** -- a standalone benchmark that
  ...
  here is a synthetic 128 BPM beat (it never opens a capture device):
      ...
      python -m lumen.preview_window                # active scene, Tk window
      python -m lumen.preview_window --fullspeed    # ignore the FPS cap, measure max throughput
      python -m lumen.preview_window --headless     # no window, just print stats (for SSH / CI)
      python -m lumen.preview_window --effect 3     # force one effect onto every fixture
...
def _anchor_db_to_backend_root() -> Path:
    """`settings.db_path` is relative ("lumen.db"), so `lumen.db.engine` points
    ...
    Re-point the engine at `backend/lumen.db` (this file is
    backend/src/lumen/preview_window.py) so we always read the same DB the
    server writes, no matter the cwd. An absolute LUMEN_DB_PATH is left alone.
    """
...
def run_window(bench: Benchmark, target_fps: float, reload_cb) -> None:
    ...
    root = tk.Tk()
    root.title("lumen render-speed preview (no webserver)")
...
class LivePreviewWindow:
    ...
    def __init__(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="lumen-preview-window", daemon=True
        )
        self._thread.start()
    ...
    def _run(self) -> None:
        ...
        root.title("lumen live preview -- render loop, no webserver")
        root.configure(bg="#111")
...
def main() -> None:
    ...
    if not Path(db_file).exists():
        raise SystemExit(
            ...
            "or set LUMEN_DB_PATH to an absolute path"
        )
    print(f"db: {db_file}")
    init_db()
    force = args.effect
